...
def generate_data(i, meanp, covp, meann, covn, N):
    xd = []
    yd = []
    np.random.seed(i)
    for n in range(N):
...
err_log = []
for i in range(128):
    x_train, y_train = generate_data(i, mean_p, cov_p, mean_n, cov_n, N_train)
    x_test, y_test = generate_data(i, mean_p, cov_p, mean_n, cov_n, N_test)
